...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if let data = defaults.objectForKey("game") as? NSData {
            MinesweeperGame.currentGame = NSKeyedUnarchiver.unarchiveObjectWithData(data) as? MinesweeperGame
        }
    ...
    func applicationDidEnterBackground(application: UIApplication) {
        ...
        if MinesweeperGame.currentGame != nil {
            let data = NSKeyedArchiver.archivedDataWithRootObject(MinesweeperGame.currentGame!)
